...
class FullConnected(Base):
    ...
    def __init__(self, input_size, output_size, activator, r, rate, ps):
        ...
        :param output_size: The output size of this layer.
    ...
    def forward(self, x):
        ...
        # HU=h(U)=U^P           【2】
        self.au = self.activator.forward(self.u)
        # Y=f(HU)               【3】
        self.output = High.forward(self.au, self.ps, self.step)
    ...
    def backward(self, grad_y):
        ...
        # =dE/dY dY/DHU
        # =GY f'(HU)
        h_delta = High.backward(grad_y, self.au, self.ps, self.step)
        delta = h_delta * self.activator.backward(self.u)
